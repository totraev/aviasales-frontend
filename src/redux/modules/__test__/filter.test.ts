import filterReducer, {
  ...
  setCurrencies,
  toggleAllTransfers,
  toggleCurrentTransfer,
  toggleOnlyCurrentTransfer,
} from '../filter';
...
describe('Filter reducer', () => {
  ...
  it('Should set currencies', () => {
    const state = filterReducer(initialState, setCurrencies({
      eur: 0.5,
      rub: 1,
      usd: 2,
    }));
  ...
  it('Should select all checkboxes', () => {
    const state = filterReducer(initialState, toggleAllTransfers(true));

    expect(state.transfers.every((value) => value)).toBeTruthy();
  });

  it('Should select checkbox', () => {
    const state = filterReducer(initialState, toggleCurrentTransfer(1, true));

    expect(state.transfers[1]).toBeTruthy();
  });

  it('Should check only current checkbox', () => {
    const firstState = filterReducer(initialState, toggleCurrentTransfer(1, true));
    const secondState = filterReducer(firstState, toggleCurrentTransfer(2, true));
    const state = filterReducer(secondState, toggleOnlyCurrentTransfer(0, true));
